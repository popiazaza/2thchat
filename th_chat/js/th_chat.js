...
nzchatobj(function () {
  nzchatobj("#nzchatmessage").on("keydown", function (event) {
    if (event.key == "Enter") {
      nzSend();
      ...
      if (nzchatobj(".nzchatrow:last").length) {
        nzCommand("edit", nzchatobj(".nzchatrow:last").attr("id").substring(7));
      }
    }
  });
  nzchatobj("#nzchatmessage").on("paste", function (e) {
  ...
  nzchatobj("#nzimgup").on("click", function () {
    nzchatobj("#nzimgupload").trigger("click");
  });
  nzchatobj("#nzimgupload").on("change", function () {
  ...
  nzchatobj(".nzchat_general").on("click", function () {
    nzChatList = 0;
    nzTouid(0);
    nzchatobj("#nzchatolcontent").html(
      '<div style="text-align: center;margin-top: 140px;"><img src = "source/plugin/th_chat/images/loading.svg"></div>'
    );
    nzLoadTextInit();
    nzchatobj(".nzchat_room").removeClass("nzactive");
    nzchatobj(this).addClass("nzactive");
  });
  nzchatobj(".nzchat_whisper").on("click", function () {
    nzChatList = 1;
    nzchatobj("#nzchatolcontent").html(
      '<div style="text-align: center;margin-top: 140px;"><img src = "source/plugin/th_chat/images/loading.svg"></div>'
    );
  ...
  nzchatobj("#nzchatcontent").on("scroll", function () {
...
function nzNotice() {
  ...
  nzchatobj(".nzquoteboxi").html(
    '<div><span class="nzquoteboxh">แก้ไขประกาศ</span>: ' +
      nzchatobj("#nzchatnotice").html() +
      '</div><div class="nzcancel" onclick="nzCancelCommand()" title="ยกเลิก"></div>'
  );
  nzchatobj(".nzquoteboxo").show();
  nzchatobj("#nzchatcontent").css(
    "height",
    nzsetting.chatheight - nzchatobj(".nzquoteboxo").height()
  );
  nzScrollChat(true);
  nzchatobj("#nzchatmessage").val(nzchatobj("#nzchatnotice").text());
  nzchatobj("#nzchatmessage").trigger("focus");
}

function nzSend() {
  var data = nzchatobj("#nzchatmessage").val().trim();
  if (data === "") {
    return false;
  }
  nzchatobj("#nzchatmessage").val("");
...
function nzCommand(command, xid) {
  if (command == "") {
    ...
    if (command == "del") {
      ...
      nzcommandz = "edit " + xid;
      nzchatobj(".nzquoteboxi").html(
        '<div><div class="nzquoteboxh">แก้ไขข้อความ</div>' +
      ...
      nzScrollChat(true);
      nzchatobj("#nzchatmessage").val(
        nzchatobj(".nzquoteboxi .nzinnercontent").text().trim()
      );
      nzchatobj("#nzchatmessage").trigger("focus");
      return;
    } else if (command == "ban") {
      var show = "แบน";
      var showid =
        " " + nzchatobj("#nzolpro_" + xid).text() + "(UID: " + xid + ")";
...
function nzLoadTextInit() {
  nzchatobj.post(
    ...
    function (data) {
      ...
      nzchatobj("#nzchatcontent").html(
        '<div class="nzcallrow">' +
          data.datahtml +
          '<div id="afterme"></div></div>'
      );
      nzScrollChat(true);
      nzchatobj(".nzinnercontent img").one("load", function () {
        nzScrollChat();
...
function nzQuota(i) {
  ...
  if (nzchatobj("#nzrows_" + i + " .nzuserat2")[0]) {
    nzchatobj(".nzquoteboxi").html(
      '<div class="nzinnercontent"><div class="nzblockquote">' +
        nzchatobj("#nzrows_" + i + " .nzuserat2")[0].outerHTML +
        ": " +
        nzchatobj("#nzchatcontent" + i).html() +
        '</div></div><div class="nzcancel" onclick="nzCancelCommand()" title="ยกเลิก"></div>'
    );
  } else {
    nzchatobj(".nzquoteboxi").html(
      '<div class="nzinnercontent"><div class="nzblockquote">' +
...
function nzTouid(i) {
  ...
  if (i > 0) {
    nzchatobj(".nzquoteboxp").html(
      '<div style="margin: 0 auto;line-height: 30px;"><span class="nzquoteboxh">แชทส่วนตัวกับ</span> <img src="uc_server/avatar.php?uid=' +
        i +
        '&size=small" class="nzchatavatar" width="32" height="32" onerror="this.src=\'uc_server/images/noavatar_small.gif\';"> ' +
        nzchatobj(".nzat_" + i).last()[0].outerHTML +
        '</div><div class="nzcancel" onclick="nzTouid(0)" title="ยกเลิก"></div>'
    );
...
function nzCancelCommand() {
  nzchatobj(".nzquoteboxi").hide();
  if (nzcommandz.substring(0, 4) == "edit") {
    if (
      nzchatobj(".nzquoteboxi .nzinnercontent").text() ==
      nzchatobj("#nzchatmessage").val()
    ) {
      nzchatobj("#nzchatmessage").val("");
    }
